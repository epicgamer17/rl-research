...
class DistributedLearnerConfig(DistributedConfig):
    def __init__(self, config_dict):
        ...
        self.samples_queue_size: int = self.parse_field("samples_queue_size", 16, wrapper=int)
        self.updates_queue_size: int = self.parse_field("updates_queue_size", 16, wrapper=int)
        self.push_params_interval: int = self.parse_field("push_params_interval", 100, wrapper=int)
...
class DistributedActorConfig(DistributedConfig):
    def __init__(self, config_dict):
        ...
        self.poll_params_interval: int = self.parse_field("poll_params_interval", 100, wrapper=int)
