        ...
        while not termination and not truncation:
            ...
            if active_player == 0:
                predictions = (
                    agent1.policy(
                        torch.tensor(
                            observation["observation"], dtype=torch.float32
                        ).reshape(1, in_size)
                    )
                    .detach()
                    .numpy()[0]
                ...
                sample, policy = modelselect.select_actions(
                    predictions,
                    info=torch.from_numpy(observation["action_mask"]).type(torch.float),
                    mask_actions=True,
                )
            else:
...
class NFSPWrapper:
    def __init__(self, env):
        ...
        self.observations = {
            "info_state": [
                self.state.observation_tensor(_)
                for _ in range(self.state.num_players())
            ],
            "legal_actions": [
                self.state.legal_actions(_) for _ in range(self.state.num_players())
            ],
        }
    ...
    def obs(self):
        if not self.state.is_terminal():
            self.observations["info_state"][self.state.current_player()] = (
                self.state.observation_tensor(self.state.current_player())
            )
            self.observations["legal_actions"][self.state.current_player()] = np.stack(
                self.state.legal_actions(self.state.current_player())
            )
        ...
        if self.state.is_terminal():
            return (
                {
                    "observation": self.state.observation_tensor(
                        int(self.agent_selection)
                    ),
                    "action_mask": np.stack([0, 1, 2, 3]),
                },
                (
        ...
        return (
            ...
            (
                self.state.player_reward(self.traverser)
                if self.traverser is not None
                else self.state.player_return(self.state.current_player())
            ),
            self.state.is_terminal(),
            False,
            "OPENSPIEL",
        ...
        if nodes >= checkpoint * max_nodes:
            for i in range(len(agents)):
                ...
                if not os.path.exists(
                    "checkpoints/" + game_string + "/nfsp/" + str(i) + "/" + str(nodes)
                ):
                    os.makedirs(
                        "checkpoints/"
                        + game_string
...
class WrapperEnv:
    ...
    def obs(self):
        return (
            {
                "observation": self.state.observation_tensor(int(self.agent_selection)),
                "action_mask": np.stack(
...
class EvalWrapper:
    def __init__(self, game, agent, in_size, out_size, select_model):
        ...
        self.observation_shape = np.array(
            [i for i in range(in_size)], dtype=np.float32
        )  # shape[0] with be in_size for agent checking
    ...
    def reset(self, seed=None):
        ...
        if self.starting_player == 0:
            return obs_mask["observation"], {
                "legal_moves": self.game.state.legal_actions()
            }
        else:
            obs_mask["action_mask"][
                0
            ] = 0  # can't fold as player 1 during cards delt must fix, must give players an id.
            preds = (
                self.agent.policy(
                    torch.tensor(obs_mask["observation"], dtype=torch.float32).reshape(
                        1, self.insize
                    )
                )
                .detach()
            ...
            action, policy = self.select_model.select_actions(
                preds,
                info=torch.from_numpy(obs_mask["action_mask"]).type(torch.float),
                mask_actions=True,
            )
...
class ActionSpaceSample:
    def __init__(self, game, out_size):
        ...
        self.shape = torch.tensor(
            [i for i in range(out_size)]
        ).shape  # shape[0] with be out_size for agent checking
...
class NFSPEvalWrapper:
    ...
    def reset(self, seed=None):
        ...
        if self.starting_player == 0:
            ...
            construct = copy.deepcopy(
                self.game
            )  # can't fold as player 1 during cards delt must fix, must give players an id.
    ...
    def step(self, action):
        ...
        if self.game.state.is_terminal():
            return (
                self.game.state.observation_tensor(0),
                reward,
