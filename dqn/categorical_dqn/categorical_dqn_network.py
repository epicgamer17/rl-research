...
class Network(Model):
        ...
        **kwargs
        ...
        if self.has_dense_layers:
            self.dense_layers = DenseStack(
                widths, self.config.activation, self.config.kernel_initializer
            )

        self.dense_output = tf.keras.layers.Dense(
            output_size * self.config.atom_size,
            activation=None,
            kernel_initializer=prepare_kernel_initializers(config.kernel_initializer),
        )

        self.distributions = tf.keras.layers.Reshape(
            (output_size, self.config.atom_size)
        )
    ...
    def call(self, inputs, training=False):
        ...
        q = self.dense_output(x)
        q = self.distributions(q)
