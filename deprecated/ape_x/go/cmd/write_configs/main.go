...
func CreateConfigsRemote(
	...
	commands := []string{
		...
		fmt.Sprintf("python3 config_generator.py --world_size %d --master_addr %s --replay_addr %s --storage_addr %s --rpc_port %d --actor_base %s --learner_base %s --actor_output %s --learner_output %s", worldSize, config.MasterHost, config.ReplayHost, config.StorageHost, config.RPCPort, actorConfigBaseFilename, learnerConfigBaseFilename, config.ActorConfigFilename, config.LearnerConfigFilename),
	}
