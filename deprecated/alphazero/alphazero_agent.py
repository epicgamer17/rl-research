...
class AlphaZeroAgent(MARLBaseAgent):
    def __init__(
        self,
        env,
    ...
    def monte_carlo_tree_search(self, env, state, info):
        ...
        root.to_play = env.agent_selection
        ...
        for _ in range(self.config.num_simulations):
            ...
            while node.expanded():
                ...
                obs, reward, terminated, truncated, info = mcts_env.last()
                agent_id = mcts_env.agent_selection
                current_player = mcts_env.agents.index(agent_id)
                _, info = process_petting_zoo_obs(obs, info, current_player)
    ...
    def play_game(self):
        ...
        while not done:
            ...
            if game.length < self.config.num_sampling_moves:  # and (not self.is_test)
            ...
            next_state, reward, terminated, truncated, next_info = self.env.last()
            agent_id = self.env.agent_selection
            current_player = self.env.agents.index(agent_id)
