...
import numpy as np
import alphazero_agent as agent
...
class Node:
    ...
    #         probabilities, value = agent.predict_single(self, self.game)
        
    #     max_u, best_a = -float("inf"), -1
    #     for action in self.action_possible:
    #         exepected_q_value = agent.predict(self.game)
    #         u = Q[s][action] + c_puct*P[s][a]*sqrt(sum(N[s]))/(1+N[s][a])
    #         if u>max_u:
    #             max_u = u
    #             best_a = a
    #     a = best_a
    ...
    #     current.visits += 1      
    #     dad = current
    #     value_propagation = 0000000000000000000000000000
            
    #     while dad.parent:
    #         dad = dad.parent
    #         dad.visits += 1
    #         dad.propagation_value += value_propagation
