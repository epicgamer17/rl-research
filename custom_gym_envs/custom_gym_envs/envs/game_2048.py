...
class Game2048Env(gym.Env):
    ...
    def __init__(self) -> None:
        ...
        assert self.reward_type == "raw" or (
            self.reward_type == "merged_tiles_plus_log_max_tile_num"
            and self.reward_normalize is False
        )
        ...
        self.chance = 0
        self.chance_space_size = 32  # 32 for 2 and 4, 16 for 2
        self.max_tile_num = 0
        self.size = 4
        self.w = self.size
        self.h = self.size
        ...
        self._observation_space = spaces.Box(
            0, 1, (self.squares, self.w, self.h), dtype=int
        )
        self._reward_range = (0.0, self.max_tile)
        ...
        if self.num_of_possible_chance_tile > 2:
            self.possible_tiles = np.array(
                [2 ** (i + 1) for i in range(self.num_of_possible_chance_tile)]
            )
            self.tile_probabilities = np.array(
                [
                    1 / self.num_of_possible_chance_tile
    ...
    def reset(
        self, seed=None, options=None, init_board=None, add_random_tile_flag=True
    ):
        ...
        info = {
            "legal_moves": action_mask_to_legal_moves(action_mask),
            "to_play": 1,
            "chance": self.chance,
        }
    ...
    def step(self, action):
        ...
        if self.reward_type == "merged_tiles_plus_log_max_tile_num":
            reward_merged_tiles_plus_log_max_tile_num = float(
        ...
        if self.reward_type == "merged_tiles_plus_log_max_tile_num":
            # reward = to_ndarray(reward_merged_tiles_plus_log_max_tile_num).astype(
            #     np.float32
            # )
            reward = reward_merged_tiles_plus_log_max_tile_num
    ...
    def move(self, direction, trial=False):
        ...
        if direction in [0, 2]:
            for y in range(self.h):
                ...
                move_reward += reward
                if (
                    old_col != new_col and not trial
                ):  # Update the board if it's not a trial move
                    for x in range_x:
    ...
    def shift(self, row, merge_direction):
        ...
        start, stop, step = (
            (0, len(non_zero_row), 1)
            if merge_direction == 0
            else (len(non_zero_row) - 1, -1, -1)
        )
    ...
    def add_random_tile(
        self,
        possible_tiles: np.array = np.array([2, 4]),
        ...
        if len(possible_tiles) != len(tile_probabilities):
            raise ValueError(
                "Length of possible_tiles and tile_probabilities must be the same"
            )
        if np.sum(tile_probabilities) != 1:
            raise ValueError("Sum of tile_probabilities must be 1")

        tile_val = self.np_random.choice(possible_tiles, 1, p=tile_probabilities)[0]
        tile_idx = np.where(possible_tiles == tile_val)[0][
            0
        ]  # get the index of the tile value
        empty_location = self.get_empty_location()
        if empty_location.shape[0] == 0:
            self.should_done = True
            return
    ...
    def save_render_output(
        self, replay_name_suffix: str = "", replay_path=None, format="gif"
    ):
...
def encode_board(flat_board, num_of_template_tiles=16):
    ...
    tile_values[0] = (
        0  # The first tile represents an empty slot, so set its value to 0.
    )
    ...
    layered_board = np.repeat(
        flat_board[:, :, np.newaxis], num_of_template_tiles, axis=-1
    )
