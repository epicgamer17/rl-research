...
class MuZeroAgent(MARLBaseAgent):
    ...
    def learn(self):
        ...
        predictions_tensor = self._stack_predictions(
            values,
            rewards,
            policies,
            to_plays,
    ...
    def _stack_predictions(
        ...
        # Stack the results into (K+1, B, ...) tensors, then permute to (B, K+1, ...)
        predictions = {
            "values": torch.stack(values).permute(
                1, 0, *range(2, len(values[0].shape) + 1)
            ),
            "rewards": torch.stack(rewards).permute(
                1, 0, *range(2, len(rewards[0].shape) + 1)
            ),
            "policies": torch.stack(policies).permute(
                1, 0, *range(2, len(policies[0].shape) + 1)
            ),
            "to_plays": torch.stack(to_plays).permute(
                1, 0, *range(2, len(to_plays[0].shape) + 1)
            ),
            "latent_states": torch.stack(latent_states).permute(
                1, 0, *range(2, len(latent_states[0].shape) + 1)
            ),
        }
        ...
        if self.config.stochastic:
            predictions.update(
                {
                    "latent_afterstates": torch.stack(latent_afterstates).permute(
                        1, 0, *range(2, len(latent_afterstates[0].shape) + 1)
                    ),
                    "latent_code_probabilities": torch.stack(
                        latent_code_probabilities
                    ).permute(
                        1, 0, *range(2, len(latent_code_probabilities[0].shape) + 1)
                    ),
                    "encoder_onehots": torch.stack(encoder_onehots).permute(
                        1, 0, *range(2, len(encoder_onehots[0].shape) + 1)
                    ),
                    "encoder_softmaxes": torch.stack(encoder_softmaxes).permute(
                        1, 0, *range(2, len(encoder_softmaxes[0].shape) + 1)
                    ),
                    "chance_values": torch.stack(chance_values).permute(
                        1, 0, *range(2, len(chance_values[0].shape) + 1)
                    ),
                }
            )
