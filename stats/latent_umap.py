...
try:
    ...
    raise ImportError(
        "umap-learn is required. Please install it with `pip install umap-learn`."
    )
...
class LatentUMAPVisualizer:
    ...
    def _adjust_n_neighbors(self, n_samples: int):
        """Adjusts n_neighbors if it's larger than the dataset size to avoid warnings."""
        if n_samples < 2:
            return  # UMAP needs at least 2 samples, but we'll let it fail or handle elsewhere

        # UMAP requires n_neighbors < n_samples
        adjusted_neighbors = min(self.default_n_neighbors, n_samples - 1)
        # Ensure at least 2 neighbors if possible, though UMAP might want more
        adjusted_neighbors = max(2, adjusted_neighbors)

        if self.reducer.n_neighbors != adjusted_neighbors:
            # print(f"Adjusting UMAP n_neighbors from {self.reducer.n_neighbors} to {adjusted_neighbors} for {n_samples} samples")
            self.reducer.n_neighbors = adjusted_neighbors
    ...
    def fit(self, latents: Union[torch.Tensor, np.ndarray]):
        ...
        self._adjust_n_neighbors(processed.shape[0])
