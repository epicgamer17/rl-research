...
from agent_configs.a2c_config import A2CConfig
...
class A2CNetwork(nn.Module):
    def __init__(
        self, config: A2CConfig, input_shape: Tuple[int], output_size: int, discrete
    ):
        if discrete:
        ...
        if config.shared_networks:
            self.actor = ActorNetwork(config, config.joint_input_shape, output_size, discrete)
            self.critic = CriticNetwork(config, config.joint_input_shape )
            self.mainStream = MainStack(config, input_shape, config.joint_output_size)
        else:
