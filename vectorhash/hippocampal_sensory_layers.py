...
class HippocampalSensoryLayer:
    ...
    def __str__(self):
        return (
            self.__class__.__name__ + f" (input_size={self.input_size}, N_h={self.N_h})"
        )
...
class ComplexExactPseudoInverseHippocampalSensoryLayer(HippocampalSensoryLayer):
    def __init__(self, input_size, N_h, N_patts, hbook, device=None):
        ...
        self.W_hs = torch.zeros((2 * N_h, input_size), device=self.device)
        """Sensory to hippocampal weight matrix. Shape: `(N_h x input_size)`
        """

        self.W_sh = torch.zeros((input_size, 2 * N_h), device=self.device)
        """Hippocampal to sensory weight matrix. Shape: `(input_size x N_h)`
        """
    ...
    def learn(self, h, s):
        self.sbook[self.size] = s
        self.hbook[self.size] = torch.cat([h.real, h.imag])
        # self.W_hs = torch.linalg.lstsq(self.hbook, self.sbook).solution
        # self.W_sh = torch.linalg.lstsq(self.sbook, self.hbook).solution
        self.W_hs = self.hbook.T @ self.sbook.pinverse().T
        self.W_sh = self.sbook.T @ self.hbook.pinverse().T
    ...
    def hippocampal_from_sensory(self, S):
        ...
        concatted = S @ self.W_hs.T
        return torch.complex(concatted[:, : self.N_h], concatted[:, self.N_h :])
    ...
    def sensory_from_hippocampal(self, H):
        ...
        return torch.cat([H.real, H.imag], dim=1) @ self.W_sh.T
...
class ComplexExactPseudoInverseHippocampalSensoryLayerComplexScalars(
    HippocampalSensoryLayer
):
    def __init__(self, input_size, N_h, N_patts, hbook, device=None):
        super().__init__(input_size, N_h, device)
...
class HebbianHippocampalSensoryLayer(HippocampalSensoryLayer):
    ...
    def __str__(self):
        return (
            super().__str__()
            + f" (use_h_fix={self.use_h_fix}, scaling_updates={self.scaling_updates})"
        )
...
class ComplexIterativeBidirectionalPseudoInverseHippocampalSensoryLayer(
    HippocampalSensoryLayer
):
    def __init__(
        self,
        ...
        self.epsilon_hs = epsilon_hs
        self.epsilon_sh = epsilon_sh
        hidden_size_sh = 2 * self.N_h * self.hidden_layer_factor
        if hidden_size_sh == 0:
            hidden_size_sh = 2 * self.N_h
        else:
            self.hidden_sh = (
                torch.rand((hidden_size_sh, 2 * self.N_h), device=device) - 0.5
            )
        self.W_sh = torch.zeros((self.input_size, hidden_size_sh), device=device)
        ...
        self.W_hs = torch.zeros((2 * self.N_h, hidden_size_hs), device=device)
        if epsilon_hs == None:
            self.epsilon_hs = hidden_size_hs
        else:
            self.epsilon_hs = epsilon_hs
    ...
    def learn(self, h, s):
        self.learned_pseudo_inverse_hs(
            ...
            output=torch.cat([h.real, h.imag]),
        )
        self.learned_pseudo_inverse_sh(
            input=(
                torch.sigmoid(self.hidden_sh @ torch.cat([h.real, h.imag]))
                if self.hidden_layer_factor != 0
                else torch.cat([h.real, h.imag])
            ),
            output=s,
        )
    ...
    def sensory_from_hippocampal(self, H):
        ...
        if self.hidden_layer_factor != 0:
            hidden = torch.sigmoid(
                torch.cat([H.real, H.imag], dim=1) @ self.hidden_sh.T
            )
            return hidden @ self.W_sh.T
        else:
            return torch.cat([H.real, H.imag], dim=1) @ self.W_sh.T
...
class ComplexIterativeBidirectionalPseudoInverseHippocampalSensoryLayerComplexScalars(
    HippocampalSensoryLayer
):
    def __init__(
        self,
        ...
        if hidden_size_sh == 0:
            ...
            self.hidden_sh = (
                torch.rand(
                    (hidden_size_sh, self.N_h), device=device, dtype=torch.complex64
                )
                - 0.5
            )
        self.W_sh = torch.zeros(
            (self.input_size, hidden_size_sh), device=device, dtype=torch.complex64
        )
        ...
        if epsilon_sh == None:
            ...
            self.epsilon_sh = epsilon_sh
        self.inhibition_matrix_sh = torch.eye(
            hidden_size_sh, device=device, dtype=torch.complex64
        ) / (self.epsilon_sh**2)
        ...
        self.W_hs = torch.zeros(
            (self.N_h, hidden_size_hs), device=device, dtype=torch.complex64
        )
        if epsilon_hs == None:
            self.epsilon_hs = hidden_size_hs
        else:
            self.epsilon_hs = epsilon_hs
        self.inhibition_matrix_hs = torch.eye(
            hidden_size_hs, device=device, dtype=torch.complex64
        ) / (self.epsilon_hs**2)

    @torch.no_grad()
    def complex_sigmoid(self, x: torch.Tensor):
    ...
    def learn(self, h, s):
        ...
        self.learned_pseudo_inverse_hs(
            input=(
                self.complex_sigmoid(self.hidden_hs @ s)
                if self.hidden_layer_factor != 0
                else s
            ),
            output=h,
        ...
        self.learned_pseudo_inverse_sh(
            input=(
                self.complex_sigmoid(self.hidden_sh @ h)
                if self.hidden_layer_factor != 0
                else h
            ),
            output=s,
    ...
    def sensory_from_hippocampal(self, H):
        ...
        if self.hidden_layer_factor != 0:
            hidden = self.complex_sigmoid(H @ self.hidden_sh.T)
            return hidden @ self.W_sh.T
        else:
    ...
    def hippocampal_from_sensory(self, S):
        ...
        if self.hidden_layer_factor != 0:
            hidden = self.complex_sigmoid(S @ self.hidden_hs.T)
            return hidden @ self.W_hs.T
        else:
            return S @ self.W_hs.T
