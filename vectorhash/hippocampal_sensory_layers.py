...
class ComplexIterativeBidirectionalPseudoInverseHippocampalSensoryLayer(
    ...
    def __init__(
        ...
        if hidden_size_sh == 0:
            ...
            self.hidden_sh = torch.rand((hidden_size_sh, self.N_h), device=device, dtype=torch.complex64) - 0.5
        self.W_sh = torch.zeros((self.input_size, hidden_size_sh), device=device, dtype=torch.complex64)

        if epsilon_sh == None:
            self.epsilon_sh = hidden_size_sh
        else:
            self.epsilon_sh = epsilon_sh
        self.inhibition_matrix_sh = torch.eye(hidden_size_sh, device=device, dtype=torch.complex64) / (
            self.epsilon_sh**2
        )
        ...
        if hidden_size_hs == 0:
            ...
            self.hidden_hs = (
                torch.rand((hidden_size_hs, self.input_size), device=device, dtype=torch.complex64) - 0.5
            )
        self.W_hs = torch.zeros((self.N_h, hidden_size_hs), device=device, dtype=torch.complex64)
        if epsilon_hs == None:
            self.epsilon_hs = hidden_size_hs
        else:
            self.epsilon_hs = epsilon_hs
        self.inhibition_matrix_hs = torch.eye(hidden_size_hs, device=device, dtype=torch.complex64) / (
            self.epsilon_hs**2
        )
