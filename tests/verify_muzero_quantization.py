...
def verify_quantization(enable_quantization=True):
    ...
    agent = MuZeroAgent(env=MockEnv(), config=config, name="test_quant", device="cpu")
    ...
    for name, mod in agent.target_model.named_modules():
        if isinstance(
            mod,
            (torch.nn.quantized.dynamic.Linear, torch.ao.nn.quantized.dynamic.Linear),
        ):
    ...
    print(
        f"Found {quantized_linear_count} quantized, {float_linear_count} float linear layers in target_model."
    )

    if enable_quantization:
        assert (
            quantized_linear_count > 0
        ), "target_model should have quantized layers when enabled!"
        assert (
            float_linear_count == 0
        ), "target_model should not have float linear layers when quantized!"
    else:
        assert (
            quantized_linear_count == 0
        ), "target_model should NOT have quantized layers when disabled!"
        assert (
            float_linear_count > 0
        ), "target_model should have float linear layers when disabled!"
    ...
    if enable_quantization and first_linear:
        ...
        if target_linear:
            ...
            weights_expected = torch.full(
                (target_linear.out_features, in_features), 0.5
            )
