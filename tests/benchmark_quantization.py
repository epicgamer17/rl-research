...
def benchmark_speedup():
    ...
    float_model = agent_float.target_model # Use target model for fairness, though model is same structure if not quantized
    float_model.eval()

    # Verify it is float
    is_q = False
    for m in float_model.modules():
        if isinstance(m, (torch.nn.quantized.dynamic.Linear, torch.ao.nn.quantized.dynamic.Linear)):
            is_q = True
