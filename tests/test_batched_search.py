...
class MockInference:
    ...
    def initial(self, state, model=None):
        if self.mode == "deterministic_tree":
            ...
            # Random / Stress
            B = (
                state.shape[0] if state.dim() > 1 else 1
            )  # State might be (10,) or (B, 10)
            val = torch.rand(1)
...
def test_equivalence_iterative_vs_batched_basics(batch_size):
    ...
    val_iter, policy_iter, target_iter, action_iter, _ = mcts_iter.run(
        state, info, 0, inference_fns
    )
    ...
    val_batch, policy_batch, target_batch, action_batch, _ = mcts_batch.run(
        state, info, 0, inference_fns
    )
    ...
    assert (
        action_iter == action_batch
    ), f"Action Mismatch: {action_iter} vs {action_batch}"
    assert np.isclose(
        val_iter, val_batch, atol=1e-5
    ), f"Value Mismatch: {val_iter} vs {val_batch}"
    ...
    if isinstance(target_batch, dict):
        ...
        p_batch = (
            target_batch.numpy() if hasattr(target_batch, "numpy") else target_batch
        )

    assert np.allclose(
        p_iter, p_batch, atol=1e-5
    ), f"Policy Mismatch: {p_iter} vs {p_batch}"
...
def test_batched_convergence_consistency(batch_size, virtual_loss):
    ...
    assert (
        best_action == 2
    ), f"Failed to converge to action 2 with batch_size={batch_size}, VL={virtual_loss}. Policy: {target_policy}"
...
def test_gumbel_batched_vs_iterative():
    ...
    assert (
        action_batch == 2
    ), f"Action mismatch: {action_iter} vs {action_batch} vs target: 2. Policies: {target_iter} vs {target_batch}"
...
def test_regression_no_negative_visits():
    ...
    mcts.backpropagator.backpropagate = types.MethodType(
        backprop_capture, mcts.backpropagator
    )
...
def test_regression_clean_backprop():
    ...
    def checked_backprop(
        ...
        return original_backprop(
            search_path, leaf_value, leaf_to_play, min_max_stats, config
        )

    mcts.backpropagator.backpropagate = types.MethodType(
        checked_backprop, mcts.backpropagator
    )
    mcts.run(state, info, 0, mocker.get_fns())
...
def test_randomized_stress_batching(iteration):
    ...
    try:
        ...
        traceback.print_exc()
        pytest.fail(
            f"Stress test failed on iter {iteration} with config BS={batch_size}, Gumbel={gumbel}: {e}"
        )
