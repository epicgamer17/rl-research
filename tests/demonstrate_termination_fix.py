...
def run_demonstration():
    ...
    raw_rewards = torch.tensor(
        [[10.0, 50.0, 999.0, 999.0]]
    )  # 999 is noise from next game
    raw_dones = torch.tensor([[False, True, False, False]])  # State 1 is terminal
    valid_mask = torch.tensor(
        [[True, True, True, True]]
    )  # Assume all valid for this demo
    n_step = 3
    gamma = 1.0
    ...
    print(
        f"Old Target Value (V0): {v_old[0,0].item()} (Includes noise from next transition!)"
    )
    print(
        f"New Target Value (V0): {v_new[0,0].item()} (Correctly cuts off at terminal state)"
    )
    ...
    if v_old[0, 0].item() != v_new[0, 0].item():
        ...
        print(
            "to be included in the N-step return because it checked 'done' at the START of the transition"
        )
